...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    deleteCategory(state,value){
      let index = state.filters.categories.findIndex(cat=>cat.title==value)
      state.filters.categories.splice(index,1)
    },

    deleteSubCategory(state,[v1,v2]){
      let index = state.filters.categories.find(c=>c.title==v1).sub.findIndex(f=>f==v2)
  ...
  actions: {
    ...
    async getDealers({commit,state}){
      ...
      await db.collection("dealers").orderBy("addedDate").get()
        .then(snap=>{
          snap.forEach(doc=>{
            let obj = new Object({
              id:doc.id,
    ...
    async updateProduct({commit,state},[product,id,img]){
      ...
      .then(()=>{
        if(img){
          const filename = product.image.name
          const ext = filename.slice(filename.lastIndexOf("."))
          firebase.storage().ref("productimages/"+id+ext).put(product.image)
          .then(data=>{
            return data.ref.getDownloadURL()
          })
          .then(url=>{
    ...
    deleteProduct({commit,state},{id,ext}){
      firebase.storage().ref("productimages/"+id+ext).delete()
      .then(()=>{
    ...
    async saveDealer({commit,state},item){
      ...
      .then(async key=>{
        const filename = item.image.name
        const ext = filename.slice(filename.lastIndexOf("."))
        await firebase.storage().ref("dealers/"+key+ext).put(item.image)
        .then(data=>{
          return data.ref.getDownloadURL()
        })
        .then(url=>{
      ...
      await db.collection("dealers").doc(key).update({
        imageURL:imageUrl
      })
      .then(()=>{
        var obj=new Object({
    ...
    deleteDealer({commit,state},{id,ext}){
      firebase.storage().ref("dealers/"+id+ext).delete()
      .then(()=>{
        db.collection("dealers").doc(id).delete()
        .then(()=>{
    ...
    deleteFilter({commit},[filter,item1,item2]){
      if(filter=="brand"){
        ...
        db.collection("filters").doc("categories").update(
          item1,firebase.firestore.FieldValue.arrayRemove(item2)
        )
        .then(()=>{
    ...
    getAnnos({commit,state}){
      let list=[]
      db.collection("announcements").orderBy("addedDate","desc").get()
      .then(snap=>{        
        snap.forEach(doc=>{
          let obj = new Object({
            id:doc.id,
